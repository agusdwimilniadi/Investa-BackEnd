...
func TestInsertBank(t *testing.T) {
	...
	t.Run("Invalid", func(t *testing.T) {
		bankRepoInterfaceMock.On("InsertBank", mock.AnythingOfType("bank.Domain"), mock.Anything).Return(bank.Domain{}, errors.New("Error")).Once()

		domain, err := bankUseCaseInterface.InsertBank(bankDataDummyInsert, context.Background())

		assert.Equal(t, errors.New("Error"), err)
		assert.Equal(t, bank.Domain{}, domain)

	})
}

func TestGetAllDataBank(t *testing.T) {
